...
ZenPub 现在支持高级缓存功能，可以让应用记住用户上次打开的文件和浏览位置，并在下次启动时通过恢复提示框询问用户是否需要恢复到该状态。
...
1. **恢复提示**：应用启动时会显示恢复提示框，询问是否恢复上次编辑状态
2. **状态恢复**：点击"恢复"按钮，系统会自动加载上次编辑的文件并还原光标位置
3. **新建文档**：点击"新建文档"按钮，会创建一个空白文档并清除缓存
4. **异常处理**：
   - 当没有可恢复的编辑记录时，不显示提示框直接进入新窗口
   - 当恢复过程中发生错误时，给出友好的错误提示并自动转入新建空白文档状态
5. **手动清理**：在设置中点击"清理缓存"可以清除所有缓存
6. **状态指示**：顶部状态指示器显示当前状态
7. **离线使用**：一旦资源被缓存，即使离线也可以继续使用
